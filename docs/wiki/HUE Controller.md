...
Press **Convert legacy HUE nodes** and confirm. The browser performs the entire conversion locally and sends no flow, node, `hue-ultimate-config`, `knxUltimate-config`, group-address, wiring, credential, name, position or node-ID data anywhere. After a successful conversion, it opens only an editable email draft addressed to the author without navigating away from Node-RED. The draft contains only the number of converted nodes and space for optional notes; you decide whether to send it, and it is never sent automatically. The final Node-RED message offers an optional support button, and the donation page opens only when that button is clicked.
