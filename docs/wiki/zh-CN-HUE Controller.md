...
点击**转换旧版 HUE 节点**并确认。浏览器会在本地完成全部转换，不会发送任何流程、节点、`hue-ultimate-config`、`knxUltimate-config`、组地址、连线、凭据、名称、位置或 ID 数据。转换成功后，它只会打开一封发给作者的可编辑邮件草稿，而不会离开 Node-RED。草稿只包含已转换节点数量和可选备注空间；是否发送由你决定，绝不会自动发送。最终的 Node-RED 消息会提供一个可选支持按钮；只有点击该按钮时才会打开捐赠页面。
