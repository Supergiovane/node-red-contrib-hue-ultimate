...
const dptlib = require('../../optionalKnx').dptlib// require('../../optionalKnx').dptlib;
      ...
      if (Number(config.selectedModeTabNumber) === 0) {
        try {
          switch (msg.knx.destination) {
            case config.GAscene: {
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptscene)
              if (topicMapping.isTopicMode(config) && typeof msg.payload === 'number') msg.payload = { scenenumber: msg.payload, save_recall: 0 }
        ...
        try {
          if (config.GAsceneMulti !== undefined && config.dptsceneMulti !== undefined && config.rules.length !== 0 && config.GAsceneMulti === msg.knx.destination) {
            msg.payload = topicMapping.readValue(msg, dptlib, config.dptsceneMulti)
              if (topicMapping.isTopicMode(config) && typeof msg.payload === 'number') msg.payload = { scenenumber: msg.payload, save_recall: 0 }
