...
const dptlib = require('../../optionalKnx').dptlib
...
module.exports = function (RED) {
  function knxUltimateHueLight (config) {
    ...
    const node = this
    node.knxUltimateRawTelegrams = true
    ...
    // This function is called by the hue-ultimate-config.js
    node.handleSend = (msg) => {
        ...
        try {
          switch (msg.knx.destination) {
            case config.GALightSwitch:
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightSwitch)
                  ...
                  if (msg.payload === true) {
                    if (node.timerCheckForFastLightSwitch === null) {
                      ...
                      if (config.restoreDayMode === 'setDayByFastSwitchLightALL') {
                        ...
                        if (node.serverKNX && config.GADaylightSensor !== undefined && config.GADaylightSensor !== '') {
            ...
            case config.GALightDIM:
              ...
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightDIM)
            ...
            case config.GALightKelvin:
              let retMirek
              let kelvinValue = 0
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightKelvin)
              if (topicMapping.isTopicMode(config) || config.dptLightKelvin === '7.600') {
            ...
            case config.GADaylightSensor:
              node.DayTime = Boolean(topicMapping.readValue(msg, dptlib, config.dptDaylightSensor))
            ...
            case config.GALightKelvinDIM:
              if (config.dptLightKelvinDIM === '3.007') {
                ...
                // { decr_incr: 1, data: 1 } : Start increasing until { decr_incr: 0, data: 0 } is received.
                // { decr_incr: 0, data: 1 } : Start decreasing until { decr_incr: 0, data: 0 } is received.
                msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightKelvinDIM)
            ...
            case config.GALightKelvinPercentage:
              if (topicMapping.isTopicMode(config) || config.dptLightKelvinPercentage === '5.001') {
                // 0-100% tunable white
                msg.payload = 100 - topicMapping.readValue(msg, dptlib, config.dptLightKelvinPercentage)
            ...
            case config.GALightBrightness:
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightBrightness)
            ...
            case config.GALightColor:
              msg.payload = topicMapping.readValue(msg, dptlib, config.dptLightColor)
              let gamut = null
              if (
            ...
            case config.GALightBlink:
              const gaVal = topicMapping.readValue(msg, dptlib, config.dptLightBlink)
              if (gaVal) {
            ...
            case config.GALightColorCycle:
              {
                if (node.timerColorCycle !== undefined) clearInterval(node.timerColorCycle)
                const gaValColorCycle = topicMapping.readValue(msg, dptlib, config.dptLightColorCycle)
  ...
  */
    ...
    node.updateKNXLightKelvinPercentageState = function updateKNXLightKelvinPercentageState (_value, _outputtype = 'write') {
      if (config.GALightKelvinPercentageState !== undefined && config.GALightKelvinPercentageState !== '') {
        ...
        if (topicMapping.isTopicMode(config) || config.dptLightKelvinPercentageState === '5.001') {
    ...
    */
    ...
    node.updateKNXLightKelvinState = function updateKNXLightKelvinState (_value, _outputtype = 'write') {
      if (config.GALightKelvinState !== undefined && config.GALightKelvinState !== '') {
        ...
        if (topicMapping.isTopicMode(config) || config.dptLightKelvinState === '7.600') {
    ...
    node.on('input', (msg, send, done) => {
      if (topicMapping.routeInput(node, config, msg, done)) return
      try {
