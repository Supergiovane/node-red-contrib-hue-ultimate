...
module.exports = (RED) => {
  function hueConfig (config) {
    ...
    try {
      node.sysLogger = new LoggerClass({ loglevel: node.loglevel, setPrefix: 'hue-ultimate-config.js' })
    ...
    node.initHUEConnection = async () => {
      ...
      (async () => {
        try {
          ...
          node.sysLogger?.error(`Errore hue-ultimate-config: node.initHUEConnection: ${error.message}`)
          throw (error)
        }
        ...
        (async () => {
          if (node.linkStatus === 'disconnected') {
            ...
            try {
              ...
              node.sysLogger?.error(`Errore hue-ultimate-config: node.startWatchdogTimer: ${error.message}`)
            }
          }
    ...
    */
    ...
    node.addClient = (_Node) => {
      // Update the node hue device, as soon as a node register itself to hue-ultimate-config nodeClients
  ...
  RED.nodes.registerType('hue-ultimate-config', hueConfig, {
    credentials: {
