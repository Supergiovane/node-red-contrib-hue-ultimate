...
describe('Unified HUE Controller', () => {
  ...
  it('registers only the standalone controller and bridge configuration', () => {
    const pkg = require('../package.json')
    const nodeKeys = Object.keys(pkg['node-red'].nodes)
    expect(nodeKeys).to.include('hueUltimateController')
    expect(nodeKeys).to.have.members(['hueUltimateController', 'hue-ultimate-config'])

    let registration
    require('../nodes/hueUltimateController')({
      nodes: {
        registerType: (type, constructor) => { registration = { type, constructor } }
      }
    })
    expect(registration.type).to.equal('hueUltimateController')
  ...
  it('registers complete config selectors and profile defaults without legacy editor registrations', () => {
    const editorPath = path.join(projectRoot, 'nodes/hueUltimateController.html')
    const editor = fs.readFileSync(editorPath, 'utf8')
    const registrationScript = [...editor.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi)]
      .find((match) => !/\bsrc\s*=/.test(match[1]) && match[2].includes("registerType('hueUltimateController'"))
    ...
    const RED = {
      nodes: {
        registerType: (type, definition) => {
          expect(type).to.equal('hueUltimateController')
    ...
    vm.runInNewContext(registrationScript[2], {
      RED,
      window: { HueStandaloneKNXUltimateHueControllerProfiles: hueControllerProfiles },
      console,
      setTimeout,
      clearTimeout
  ...
  it('embeds every private editor template, definition, and supported locale', () => {
    ...
    supportedTypes.forEach((controllerType) => {
      ...
      expect(definition.defaults.serverHue.type, `${controllerType} Hue config`).to.equal('hue-ultimate-config')
  ...
  it('converts pasted legacy Hue flow JSON while reusing referenced config nodes by default', () => {
    const source = [
      ...
      { id: 'hue-ultimate-config', type: 'hue-ultimate-config', name: 'Hue Bridge' },
      { id: 'unused-hue-ultimate-config', type: 'hue-ultimate-config', name: 'Unrelated Hue Bridge' }
    ]
  ...
  it('can include config nodes for a different Node-RED installation without inventing credentials', () => {
    const source = [
      ...
      { id: 'hue-ultimate-config', type: 'hue-ultimate-config', name: 'Hue' }
    ]
  ...
  it('does not remove config nodes when the pasted flow has nothing to convert', () => {
    const source = [{ id: 'hue-ultimate-config', type: 'hue-ultimate-config', name: 'Hue' }]
    const result = convertLegacyHueFlow(source)
  ...
  it('covers every legacy Hue type and emits formatted pasteable JSON', () => {
    ...
    expect(result.flow.every((node) => node.type === 'hueUltimateController')).to.equal(true)
  })
  ...
  it('classifies legacy Hue nodes without treating the unified Controller as legacy', () => {
    ...
    expect(isLegacyHueNode({ type: 'hueUltimateController' })).to.equal(false)
  ...
  it('creates conversion patches locally without copying private node data', () => {
    const legacyNodes = [{
      ...
      serverHue: 'hue-ultimate-config-id',
    ...
    expect(serialized).not.to.include('hue-ultimate-config-id')
  ...
  it('changes only legacy HUE node types locally and leaves configs, properties, groups, and wires untouched', () => {
    ...
    const configNode = { id: 'hue-ultimate-config', type: 'hue-ultimate-config', name: 'Hue bridge' }
    ...
    const RED = {
      ...
      nodes: {
        ...
        getType: (type) => type === 'hueUltimateController' ? controllerDefinition : undefined,
        dirty: (value) => {
    ...
    expect(configNode).to.deep.equal({ id: 'hue-ultimate-config', type: 'hue-ultimate-config', name: 'Hue bridge' })
  ...
  it('boots the unchanged Hue Light path through the unified node', () => {
    ...
    require('../nodes/hueUltimateController')(RED)
  ...
  it('keeps configuration-node selectors fixed and the device editor profile-driven', () => {
    const editor = fs.readFileSync(path.join(projectRoot, 'nodes/hueUltimateController.html'), 'utf8')
    ...
    expect(editor).to.include('mousedown.hueUltimateControllerDevice')
    ...
    expect(editor).to.include('resources/node-red-contrib-hue-ultimate/hueControllerProfiles.js')
    expect(editor).to.include('resources/node-red-contrib-hue-ultimate/hueControllerEditorSelection.js')
    ...
    expect(editor).to.include("serverHue: { value: '', type: 'hue-ultimate-config', required: true }")
    ...
    expect(editor).to.include('window.UltimateTopicMappingEditor.apply($, $container, knxDisabled')
    expect(editor).not.to.include('hue-controller-no-knx #tabs')
    expect(editor).not.to.include('hue-controller-no-knx .hue-knx-section')
    ...
    expect(editor).to.include('Topic → msg.payload')
    ...
    expect(editor).to.include('change.hueUltimateControllerKnxVisibility')
    expect(editor).to.include("const TUTORIALS_URL = 'https://www.youtube.com/playlist?list=PL9Yh1bjbLAYrU8PsVhW4xzEug2WtVFv3E'")
    expect(editor).to.include('resources/node-red-contrib-hue-ultimate/hueControllerMigration.js')
    expect(editor).to.include('resources/node-red-contrib-hue-ultimate/hueControllerMigrationDialog.js')
    ...
    expect(editor).to.include('window.HueUltimateMigration.prompt()')
    ...
    expect(editor).to.include('migration.hasLegacyNodes()')
    ...
    expect(editor).to.include('hueUltimateController.migration_disclaimer')
    const templateStart = editor.indexOf('<script type="text/html" data-template-name="hueUltimateController">')
  ...
  it('loads light capabilities from the persisted Hue bridge during Controller bootstrap', () => {
    ...
    // use the actual persisted hue-ultimate-config reference instead.
  ...
  it('renders the Light editor without waiting for the runtime Hue resource cache', () => {
    ...
    expect(privateLightEditor).not.to.include('hueUltimate/knxultimateCheckHueConnected')
  ...
  it('keeps Light tabs visible while Node-RED temporarily exposes an empty gateway sentinel', () => {
    ...
    const controllerEditor = fs.readFileSync(
      path.join(projectRoot, 'nodes/hueUltimateController.html'),
      'utf8'
    )
    ...
    ;['en', 'it', 'de', 'fr', 'es', 'zh-CN'].forEach((locale) => {
      const translations = require(path.join(projectRoot, 'nodes', 'locales', locale, 'hueUltimateController.json'))
      expect(translations.hueUltimateController.knx_gateway_required_for_mappings, `${locale} KNX notice`).to.be.a('string').and.not.be.empty
    })
  })

  it('keeps the mapping tabs available for topics while preserving gateway selection', () => {
    const privateLightEditor = fs.readFileSync(
      path.join(projectRoot, 'scripts/hue-controller-profiles/editors/light.js'),
      'utf8'
    )
    ...
    expect(controllerEditor).to.include("$lightTabs.css('display', 'flex')")
    expect(controllerEditor).to.include(".on('change.hueUltimateControllerKnxVisibility', '#node-input-server', handleControllerKnxSelection)")
    expect(controllerEditor).to.include("$(document).off('change.hueUltimateControllerKnxVisibility', '#node-input-server')")
    ...
    expect(privateLightTemplate).not.to.include('resources/node-red-contrib-hue-ultimate/dim.png')
    expect(privateLightTemplate).not.to.include('resources/node-red-contrib-hue-ultimate/tunablewhite.png')
    expect(privateLightTemplate).not.to.include('resources/node-red-contrib-hue-ultimate/rgb.png')
  ...
  it('waits for a ready Hue catalog while keeping persisted mappings visible and disabling offline discovery', () => {
    ...
    const hueConfigRuntime = fs.readFileSync(
      path.join(projectRoot, 'nodes/hue-ultimate-config.js'),
      'utf8'
    )
    ...
    expect(controllerEditor).not.to.include('hueUltimateController.hue_section')
    ...
    expect(controllerEditor).to.include('data-i18n="hueUltimateController.hue_bridge_unreachable"')
    ...
    expect(controllerEditor).to.include(".on('change.hueUltimateControllerHueServer', '#node-input-serverHue', (event) =>")
    ...
    ;['en', 'it', 'de', 'fr', 'es', 'zh-CN'].forEach((locale) => {
      const translations = require(path.join(projectRoot, 'nodes', 'locales', locale, 'hueUltimateController.json'))
      expect(translations.hueUltimateController).not.to.have.property('hue_section')
      expect(translations.hueUltimateController.loading_hue_devices, `${locale} Hue loading message`).to.be.a('string').and.not.be.empty
      expect(translations.hueUltimateController.hue_bridge_unreachable, `${locale} Hue unreachable message`).to.be.a('string').and.not.be.empty
      expect(translations.hueUltimateController.hue_bridge_not_ready, `${locale} Hue readiness error`).to.be.a('string').and.not.be.empty
    })
  })
  ...
  it('keeps dedicated Hue nodes registered but hides them from the palette as deprecated', () => {
    ...
    hueEditors.forEach((file) => {
      ...
      expect(html.slice(noticeStart, migrationActionStart), `${file} localized notice`).to.include(
        'node-red-contrib-hue-ultimate/hueUltimateController:hueUltimateController.legacy_node_notice'
      )
      ...
      expect(html.slice(migrationVideoStart, migrationButtonStart), `${file} localized video label`).to.include('hueUltimateController.migration_video')
      ...
      expect(html.slice(migrationActionStart, tutorialStart), `${file} localized migration disclaimer`).to.include('hueUltimateController.migration_disclaimer')
    ...
    const controller = fs.readFileSync(path.join(projectRoot, 'nodes/hueUltimateController.html'), 'utf8')
    expect(controller).to.match(/category:\s*['"]HUE Ultimate['"]/)
    expect(controller).to.match(/color:\s*['"]#F2CE72['"]/)
  ...
  it('ships help, editor strings, and documentation in every supported language', () => {
    ...
    locales.forEach((locale) => {
      const messages = require(path.join(projectRoot, 'nodes/locales', locale, 'hueUltimateController.json'))
      expect(messages.common, `${locale} common labels`).to.include.keys('knx_gw', 'hue_bridge')
      expect(messages.hueUltimateController.device_function, locale).to.be.a('string').and.not.equal('')
      expect(messages.hueUltimateController.hue_bridge_unreachable, `${locale} bridge unreachable message`).to.be.a('string').and.not.equal('')
      expect(messages.hueUltimateController.hue_bridge_not_ready, `${locale} bridge readiness error`).to.be.a('string').and.not.equal('')
      expect(messages.hueUltimateController.legacy_node_notice, `${locale} legacy notice`).to.be.a('string').and.not.equal('')
      expect(messages.hueUltimateController, `${locale} migration labels`).to.include.keys(
        'migration_button',
      ...
      expect(messages.hueUltimateController.migration_deploy_notice, `${locale} backup recommendation`)
        .to.include(localizedBackupWords[locale])
      expect(Object.keys(messages.hueUltimateController.types), locale).to.have.members(supportedTypes)
      expect(fs.existsSync(path.join(projectRoot, 'nodes/locales', locale, 'hueUltimateController.html')), locale).to.equal(true)
      ...
      const localizedHelp = fs.readFileSync(path.join(projectRoot, 'nodes/locales', locale, 'hueUltimateController.html'), 'utf8')
      ...
      expect(localizedHelp, `${locale} help wrapper`).to.match(
        /<script\b[^>]*data-help-name="hueUltimateController"[^>]*>[\s\S]*<\/script>/i
      )
    ...
    const canonicalHelp = fs.readFileSync(path.join(projectRoot, 'nodes/hueUltimateController.html'), 'utf8')
