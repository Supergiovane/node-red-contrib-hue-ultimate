...
function createHarness (options = {}) {
  ...
  const migrationApi = {
    ...
    applyLocalMigration (red, selected) {
      ...
      nodes.forEach(node => { node.type = 'hueUltimateController' })
      return nodes.length
    },
  ...
  if (!options.missingBackup) windowObject.HueStandaloneKNXUltimateFlowMigrationBackup = backupApi
...
describe('HUE migration automatic flow backup', function () {
  it('downloads synchronously after preflight and before closing the editor or converting nodes', function () {
    ...
    harness.flush()
    assert.equal(harness.nodes[0].type, 'hueUltimateController')
