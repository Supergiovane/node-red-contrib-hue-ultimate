(function (root, factory) {
  ...
  if (root) root.HueStandaloneKNXUltimateHueControllerMigration = api
  ...
  // Every legacy device becomes the single public Controller. Keep package
  // conversion in sync with the profile catalog, including the old Controller
  // and its shared bridge configuration.
  const PACKAGE_NODE_TYPES = Object.freeze({
    'hue-config': 'hue-ultimate-config',
    knxUltimateHueController: 'hueUltimateController',
    ...Object.fromEntries(Object.keys(LEGACY_NODE_PROFILES).map(type => [type, 'hueUltimateController']))
  })
  ...
  function applyLocalMigration (RED, legacyNodes) {
    ...
    const controllerDefinition = RED.nodes.getType('hueUltimateController')
  ...
  function convertLegacyHueFlow (sourceFlow, options = {}) {
    ...
    const flow = sourceFlow.map(function (sourceNode, index) {
      ...
      const pinShape = derivePinShape(node, profile)
      node.type = 'hueUltimateController'
