(function (root, factory) {
  ...
  if (root) {
    root.HueStandaloneHueStandaloneKNXUltimateHueControllerMigrationDialog = api
  ...
  const EVENT_NAMESPACE = '.hueUltimateControllerMigrationDialog'
  const BUTTON_SELECTOR = '.hue-legacy-migrate-flow'
  const BUTTON_STYLE_ID = 'knxUltimateHueLegacyMigrationButtonStyle'
  const I18N_PREFIX = 'node-red-contrib-hue-ultimate/hueUltimateController:hueUltimateController.'
  ...
  function open (options = {}) {
    ...
    const migrationApi = options.migrationApi || (windowObject && windowObject.HueStandaloneKNXUltimateHueControllerMigration)
    ...
    const performMigration = () => {
      ...
      try {
        ...
        try {
          const backupApi = options.backupApi || windowObject.HueStandaloneKNXUltimateFlowMigrationBackup
